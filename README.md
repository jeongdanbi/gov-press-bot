...
2025년 korea.kr가 RSS(`/rss/*.xml`)를 전부 폐지(404)해, 살아있는 HTML 목록
페이지(`pressReleaseList.do`)를 스크래핑하는 방식(`korea_kr_list`)으로 전환함.
각 항목의 부처명(`span.source`)이 `dept_match`에 매칭되는 건만 수집한다.

| 기관 | 이모지 | 방식 |
|---|---|---|
| 금융위원회 | 🏦 | korea.kr HTML 목록 |
| 공정거래위원회 | ⚖️ | korea.kr HTML 목록 |
| 재정경제부 | 🏛️ | korea.kr HTML 목록 |
| 국세청 | 📊 | korea.kr HTML 목록 |
| 국토교통부 | 🏗️ | korea.kr HTML 목록 |
| 중소벤처기업부 | 🚀 | korea.kr HTML 목록 |

- 2025 정부조직 개편으로 **기획재정부 → 재정경제부**로 korea.kr 표기가 바뀌었다.
- 금융감독원은 정부부처가 아닌 특수법인이라 korea.kr(정책브리핑)에 게시되지 않아
  기본 비활성(`fss_via_all: enabled: false`). 필요 시 홈페이지 직접 스크래핑을 구현해야 함.
...
- **수집이 0건**: `python -m src.main --dry-run --source fsc` 로 확인.
  - korea.kr HTML 구조가 바뀌었을 수 있음 → `fetcher.py`의 `_fetch_korea_kr_list_page`
    셀렉터(`.list_type li`, `a[href*=pressReleaseView]`, `span.source`) 확인.
  - 부처명 표기가 바뀌었을 수 있음 → `sources.yaml`의 `dept_match` 확인
    (예: 기획재정부 → 재정경제부 개편).
  - 저빈도 부처(국세청·중기부 등)는 최근 목록에 안 잡힐 수 있음 → `list_pages` 늘리기.
